...
- *se* returns the standard errors for the estimated `SIGMA`
  variance-covariance matrix.

- *rse* returns the relative standard errors for the estimated `SIGMA`
...
- *95ci* returns the asymptotic 95% confidence intervals for the
  elements of the `SIGMA` variance-covariance matrix
  (`est +/- 1.96*se`).

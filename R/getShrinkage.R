...
#'  shr <- getShrinkage(nmOutput, output="all")

getShrinkage <- function(x, output="eta", sigdig=3) {
  ...
  eta <- signif(as.numeric(x$nonmem$problem$estimation$etashrink[seq(1,
                                                                     length(x$nonmem$problem$estimation$etashrink)-1, by=2)]), sigdig)

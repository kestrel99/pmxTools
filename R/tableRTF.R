...
#'
#' \code{tableRTF} generates an RTF table from a data frame.
#'
#' @param df A data frame.
#' @param outFile A filename for writing the table to. If \code{NULL}, writes to console.
#' @param rtfFile If \code{TRUE} (the default), then add RTF tabs (\code{\{\rtf1} to the
#' beginning and \code{\}} to the end) to generate a fully formatted RTF file.
...
#' myRTF <- tableRTF(scm$forwardSummary)
#'
#' @references \url{http://www.r-bloggers.com/another-solution-to-the-r-to-word-table-problem/}

tableRTF <- function(df, outFile=NULL, rtfFile=TRUE, boldHeader=TRUE,
                     rowNames=FALSE, ...) {
